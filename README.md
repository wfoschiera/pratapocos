...
```bash
dk bash  # starts bash inside "pratapocos" container
...
- `dkup` uses docker-compose to start 3 containers: postgres, nginx, and pratapocos.
...
- Once `dkup` is running, `dk <command>` will run `<command>` inside the `pratapocos` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
- The pratapocos container runs 3 services:
- django on port 8000
...
```bash
mkvirtualenv pratapocos -p python3  # creates a python3 virtualenv
